...
import { ChevronLeft, MoreHorizontal, Sparkles, CheckCircle, Quote, Utensils, Activity, BookOpen, Loader2 } from 'lucide-react';
...
export const TraitDetail: React.FC<TraitDetailProps> = ({ trait, onBack, onSubTraitClick }) => {
   ...
   useEffect(() => {
      const fetchExtraData = async () => {
         ...
         const { data: celebData } = await supabase
          ...
          description,
          image_url,
          traits ( trait_id, name )
        `)
            .eq('traits.name', trait.name)
            .maybeSingle();
         ...
         if (celebData) {
            setCelebrity({
               ...
               image: celebData.image_url,
               role: 'Genetic Match',
               matchLevel: '98% Similarity'
            });
         }
   ...
   // Fallback to constants if DB is empty for this specific trait
   const staticDetails = TRAIT_DETAILS[trait.name] || {
      summary: trait.description,
      subTraits: [
         { name: trait.name, label: 'GENERAL ANALYSIS', level: trait.level, icon: trait.iconName }
      ],
      celebrity: null,
      careers: []
   };

   const currentCelebrity = celebrity || staticDetails.celebrity;
   ...
   return (
      <div className="min-h-screen bg-gray-50 pb-10 animate-in slide-in-from-right duration-300">
         ...
         <div className="px-6 -mt-16 relative z-20 space-y-6">
            ...
            {/* Dynamic Recommendations / Action Plan */}
            {recommendations.length > 0 && (
               <div className="animate-in fade-in slide-in-from-bottom duration-500">
                  <h3 className="font-bold text-xl text-slate-900 mb-4 px-1 flex items-center">
                     <BookOpen size={20} className="mr-2 text-teal-500" />
                     Action Plan
                  </h3>
                  <div className="space-y-3">
                     {recommendations.map((rec, idx) => (
                        <div key={idx} className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 flex items-start space-x-4">
                           <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${rec.type === 'SUPPLEMENT' ? 'bg-orange-50 text-orange-500' :
                                 rec.type === 'EXERCISE' ? 'bg-blue-50 text-blue-500' : 'bg-teal-50 text-teal-500'
                              }`}>
                              {rec.type === 'SUPPLEMENT' ? <Utensils size={20} /> :
                                 rec.type === 'EXERCISE' ? <Activity size={20} /> : <Sparkles size={20} />}
                           </div>
                           <div>
                              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{rec.type}</p>
                              <p className="text-sm text-slate-700 leading-relaxed font-medium">{rec.advice_text}</p>
                           </div>
                        </div>
                     ))}
                  </div>
            ...
            {/* Main Sub-Trait Card */}
            {staticDetails.subTraits.length > 0 && (
               <div
                  onClick={() => onSubTraitClick(staticDetails.subTraits[0].name)}
               ...
               >
                  <div className="flex justify-between items-start mb-4">
                     <div className="w-12 h-12 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center">
                        {getIcon(staticDetails.subTraits[0].icon || 'brain', 24)}
                     </div>
                     <div className="bg-blue-600 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-blue-200 shadow-sm">
                        {staticDetails.subTraits[0].level}
                     </div>
                  </div>

                  <h3 className="text-xl font-bold text-slate-900 mb-1">{staticDetails.subTraits[0].name}</h3>
                  <p className="text-[10px] text-gray-400 font-bold tracking-widest uppercase mb-6">
                     {staticDetails.subTraits[0].label}
                  </p>

                  <LevelLabels />
                  <LevelBar level={staticDetails.subTraits[0].level} activeColor="bg-blue-600" />
               </div>
            )}

            {/* Secondary Traits Grid */}
            {staticDetails.subTraits.length > 1 && (
               <div className="grid grid-cols-2 gap-3">
                  {staticDetails.subTraits.slice(1).map((sub: any, idx: number) => (
                     <div
                        key={idx}
               ...
               <div className="animate-in slide-in-from-bottom duration-500 delay-100">
                  ...
                  <div className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 flex flex-col items-center text-center relative overflow-hidden">
                     ...
                     <div className="flex items-center justify-center space-x-3 mb-2 z-10">
                        <h3 className="text-2xl font-black text-slate-900 leading-tight">{currentCelebrity.name}</h3>
            ...
            {staticDetails.careers && staticDetails.careers.length > 0 && (
               <div className="pb-8">
                  <div className="flex justify-between items-end mb-4 px-1">
                  ...
                  <div className="relative group">
                     ...
                     >
                        {staticDetails.careers.map((career: any, idx: number) => (
                           <div
                              key={idx}
                     ...
                     {/* Navigation Dots */}
                     {staticDetails.careers.length > 1 && (
                        <div className="flex justify-center items-center space-x-1.5 -mt-2">
                           {staticDetails.careers.map((_: any, idx: number) => (
                              <div
                                 key={idx}
                                 className={`h-1.5 rounded-full transition-all duration-300 ${idx === activeCareerSlide ? 'w-4 bg-blue-600' : 'w-1.5 bg-gray-200'
                                    }`}
                              />
                           ))}
                        </div>
