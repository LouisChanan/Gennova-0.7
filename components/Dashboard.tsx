...
import { ChevronDown, Utensils, Sparkles, Activity, Check, Shield, FlaskConical, Clock, X, ArrowRight } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ onTraitClick, isTrackingMode = false }) => {
  // If isTrackingMode is true initially, we start viewing 'Leo' (Child/Tracking).
  // Otherwise we view 'Sarah' (Parent/Results).
  const [viewingProfile, setViewingProfile] = useState<'Sarah' | 'Leo'>(isTrackingMode ? 'Leo' : 'Sarah');
  const [activeCategory, setActiveCategory] = useState<string>('Talent');
  const [showLearnMore, setShowLearnMore] = useState(false);

  // Toggle between Sarah (Results) and Leo (Tracking)
  const toggleProfile = () => {
    setViewingProfile(prev => prev === 'Sarah' ? 'Leo' : 'Sarah');
  };

  // --- SHARED HEADER ---
  const Header = () => (
    <header className="flex justify-between items-center p-6 bg-transparent sticky top-0 z-10 backdrop-blur-md">
      <div>
        <p className="text-[10px] text-gray-400 font-bold tracking-widest uppercase mb-0.5">Welcome Back,</p>
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Sarah</h1>
      </div>
      ...
      >
          <div className={`w-8 h-8 rounded-full flex items-center justify-center overflow-hidden border-2 border-white shadow-sm shrink-0 ${viewingProfile === 'Leo' ? 'bg-teal-100' : 'bg-purple-100'}`}>
              {viewingProfile === 'Leo' ? (
                   // Leo Avatar (Child) - Simulated with DiceBear or similar
                  <div className="w-full h-full bg-[#8D6E63] flex items-center justify-center pt-1">
                   ...
                   // Sarah Avatar (Adult)
                   <img src={USER_PROFILE.avatarUrl} alt="Sarah" className="w-full h-full object-cover" />
              )}
          </div>
          <span className="text-sm font-bold text-slate-800">{viewingProfile === 'Leo' ? 'Leo' : 'Sarah'}</span>
                ...
                <div className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 relative overflow-hidden">
                    ...
                    <div className="space-y-0 relative z-10 pl-2">
                        ...
                        <div className="flex items-center mb-8 group">
                            <div className="w-10 h-10 rounded-full bg-teal-500 text-white flex items-center justify-center ring-4 ring-white z-10 shrink-0 shadow-sm transition-transform group-hover:scale-105">
                                <Check size={20} strokeWidth={3} />
                            </div>
                            <div className="ml-5">
                                <h3 className="font-bold text-slate-800 text-sm">Kit Activated</h3>
                                <p className="text-xs text-gray-400 font-medium mt-0.5">July 12, 2024</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center mb-8 group">
                            ...
                            <div className="ml-5">
                                ...
                                <p className="text-xs text-gray-400 font-medium mt-0.5">July 15, 2024</p>
                            </div>
                        </div>

                        {/* Step 3: Lab Processing (Current) */}
                        <div className="flex items-start mb-8">
                            <div className="w-10 h-10 rounded-full bg-teal-500 flex items-center justify-center ring-4 ring-white shadow-lg shadow-teal-200 z-10 shrink-0 relative">
                                <FlaskConical size={18} className="text-white relative z-10" />
                                <div className="absolute inset-0 rounded-full border-2 border-white opacity-40 animate-ping"></div>
                            </div>
                            <div className="ml-5 pt-0.5">
                                <h3 className="font-bold text-slate-900 text-base">Lab Processing</h3>
                                <div className="flex items-center mt-1">
                                    <span className="text-xs text-teal-600 font-bold uppercase tracking-wide mr-2">Current Stage</span>
                        ...
                        <div className="flex items-center opacity-40">
                             <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center ring-4 ring-white z-10 shrink-0">
                                <div className="w-3 h-3 bg-gray-300 rounded-full"></div>
                             </div>
                             <div className="ml-5">
                                <h3 className="font-bold text-slate-800 text-sm">Result Ready</h3>
                                <p className="text-xs text-gray-500 font-medium mt-0.5">Estimated Jul 25</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-8 bg-emerald-50/80 rounded-2xl p-4 flex items-start space-x-4 border border-emerald-100">
                        ...
                        <div className="flex-1">
                             <p className="text-sm text-emerald-900 font-bold mb-0.5">Secure Processing</p>
                             <p className="text-xs text-emerald-700 leading-relaxed">
                                Your sample is being handled with care by our experts in our certified lab.
                            </p>
                        </div>
                    </div>
                </div>
  ...
  const filteredTraits = TRAITS.filter(t => t.category === activeCategory);
  ...
  const TabButton = ({ category, label, icon: Icon }: { category: string, label: string, icon: any }) => {
    ...
    return (
      <button 
        ...
        className={`flex-1 flex items-center justify-center space-x-2 py-2.5 text-sm font-bold rounded-full transition-all duration-200 ${
            isActive 
        ...
        {topTrait ? (
          <>
            ...
            <div 
              ...
              className={`
                rounded-[32px] p-6 text-white shadow-lg relative overflow-hidden group hover:scale-[1.01] transition-transform duration-300 cursor-pointer
                ...
                >
                  <div className="flex justify-between items-start">
                     <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        trait.level === TraitLevel.GIFTED ? 'bg-pink-50 text-pink-500' :
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center text-gray-400">
            <div className="bg-gray-100 p-4 rounded-full mb-3">
                {activeCategory === 'Nutrition' ? <Utensils size={32} /> : <Activity size={32} />}
            </div>
            <p>No traits found for {activeCategory}.</p>
          </div>
        )}
